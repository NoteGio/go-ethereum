...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, _, err := ethapi.DoCall(ctx, b.backend, args.Data, nil, *b.numberOrHash, nil, 5*time.Second, b.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, _, err := ethapi.DoCall(ctx, p.backend, args.Data, nil, pendingBlockNr, nil, 5*time.Second, p.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
