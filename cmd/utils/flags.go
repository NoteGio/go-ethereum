...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node, readonly bool) ethdb.Database {
	...
	if ctx.GlobalString(SyncModeFlag.Name) == "light" {
		...
		name := "chaindata"
		chainDb, err = stack.OpenDatabaseWithOverlayAndFreezer(name, cache * 3/4, cache / 4, handles, ctx.GlobalString(AncientFlag.Name), ctx.GlobalString(OverlayFlag.Name), "", readonly)
	}
	if err != nil {
